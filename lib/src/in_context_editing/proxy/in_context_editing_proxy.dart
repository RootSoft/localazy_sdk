...
class MessageLookupProxy extends MessageLookup {
  ...
  void _detectCodeGen() {
    if (!_isDetected) {
      sendWebSocketMessage(convert.jsonEncode({'type': 'code_gen_detected', 'codeGen': 'intl_utils'}));
      _isDetected = true;
    }
  }
  ...
  String? lookupMessage(String? messageText, String? locale, String? name, List<Object>? args, String? meaning,
    ...
    try {
      ...
      var translationChangeTyped = SdkData.inContextEditingData?.getEditedData(currentLocale, name);
      if (translationChangeTyped == null) {
        return _messageLookup.lookupMessage(messageText, locale, name, args, meaning, ifAbsent: ifAbsent);
      }

      var label = Label(key: translationChangeTyped.key, value: translationChangeTyped.value);
